...
# Show params
batch = 20
params_info = paddle.summary(rtsr3,(batch, 3, 180, 320))
print(params_info)
custom_dataset = SrDataset('../data/540cut','../data/1080cut')
print('-'*75,'\ncustom_dataset images: ',len(custom_dataset))

# 用 DataLoader 实现数据加载
train_loader = paddle.io.DataLoader(custom_dataset, batch_size=batch, shuffle=False)
...
for epoch in range(epochs):
    for batch_id, data in enumerate(train_loader()):
        ...
        predicts, hisfea0, hisfea1, tag0, tag1 = rtsr3(x_data, hisfea0, hisfea1, tag0, tag1)    # 预测结果
...
paddle.jit.save(
    layer=rtsr3,
    path="jit.save/rtsr3",
    input_spec=[paddle.static.InputSpec(shape=[None, 3, 180, 320], dtype='float32')])
